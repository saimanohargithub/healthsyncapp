...
import com.example.healthsync.backend.data.local.*
...
class HealthRepository(
    ...
    fun getTotalWater(): Flow<Int?> = healthDao.getTotalWaterFlow()

    fun getCompletedChallengesCount(): Flow<Int> = callbackFlow {
        ...
        com.example.healthsync.backend.firebase.FirestoreManager.listenToJoinedChallenges(uid) { challenges ->
            trySend(challenges.filter { it.isCompleted }.size)
        }
        awaitClose { }
    }
    ...
    suspend fun logWater(amountMl: Int) = withContext(Dispatchers.IO) {
        ...
        if (userId != null) {
            com.example.healthsync.backend.firebase.FirestoreManager.syncChallengeProgress(userId, "water", total)
        }
    }
    ...
    suspend fun logSleepEntry(hours: Int, minutes: Int, score: Int) = withContext(Dispatchers.IO) {
        ...
        val dateStr = java.text.SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date(now))
        val dayStr = java.text.SimpleDateFormat("EEEE", Locale.getDefault()).format(Date(now))
        ...
        val userId = auth.currentUser?.uid
        if (userId != null) {
            com.example.healthsync.backend.firebase.FirestoreManager.saveSleepEntry(entry)
            com.example.healthsync.backend.firebase.FirestoreManager.syncChallengeProgress(userId, "sleep", hours)
        }
    }
    ...
    suspend fun logSleep(hours: Float, score: Int) = withContext(Dispatchers.IO) {
        ...
        val userId = auth.currentUser?.uid
        if (userId != null) {
            com.example.healthsync.backend.firebase.FirestoreManager.syncChallengeProgress(userId, "sleep", h)
        }
    }
    ...
    suspend fun logMood(mood: String, note: String) = withContext(Dispatchers.IO) {
        val score = com.example.healthsync.backend.engines.WellnessAnalyticsEngine.mapMoodToStress(mood)
        ...
        val userId = auth.currentUser?.uid
        if (userId != null) {
            com.example.healthsync.backend.firebase.FirestoreManager.saveMoodLog(log)
        }
    }
