...
import com.example.healthsync.backend.data.remote.UsdaApiService
import com.example.healthsync.backend.utils.FoodLabelDetectorHelper
import com.example.healthsync.backend.utils.NutritionLabelScannerHelper
import com.example.healthsync.backend.utils.NutritionCalculator
...
class MealRepository(
    ...
    suspend fun analyzeFoodImage(bitmap: Bitmap): Result<AnalysisResult> = withContext(Dispatchers.IO) {
        try {
            ...
            if (detection == null) {
                return@withContext Result.failure(Exception("Food could not be identified specifically."))
            }

            val response = api.searchFood(detection.label, 1, BuildConfig.USDA_API_KEY)
            val topFood = response.foods.firstOrNull() ?: return@withContext Result.failure(Exception("Nutrition data not found."))
    ...
    suspend fun analyzeNutritionLabel(bitmap: Bitmap, imagePath: String): Result<AnalysisResult> = withContext(Dispatchers.IO) {
        try {
            ...
            if (scanResult == null) {
                return@withContext Result.failure(Exception("Label reading failed."))
            }
    ...
    suspend fun saveMeal(meal: MealEntity) = withContext(Dispatchers.IO) {
        ...
        if (userId != null) {
            com.example.healthsync.backend.firebase.FirestoreManager.syncChallengeProgress(userId, "diet", prefs.getTodayCalories())
        }
    }
